    ...
    <div>
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "54px 22px 0" }}>
        ...
        <div style={{ marginBottom: 56 }}>
          ...
          <h2 className="tf-disp" style={{ fontSize: 28, fontWeight: 800, margin: "0 0 28px" }}>From first data export to live delivery review</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 16 }}>
            {[
              { step: "1", icon: Upload, color: "var(--thread)", title: "Revenue at Risk Diagnostic", time: "Fast to run", desc: "Send ERP exports. We map them, score data health, and produce an exposure map — committed vs blocked value, top blockers, and a pilot design." },
              { step: "2", icon: Wrench, color: "var(--amber)", title: "Paid pilot", time: "Live quickly", desc: "One site, one value stream. A live delivery review calendar, blockers with owners, and a before/after baseline you own." },
              { step: "3", icon: Activity, color: "var(--green)", title: "Annual subscription", time: "Production", desc: "Scheduled feeds, freshness indicators, per-quarter value review. Expand to additional sites or functions at renewal." },
              { step: "4", icon: TrendingUp, color: "var(--blue)", title: "Enterprise", time: "Complex accounts", desc: "Multiple ERPs, SSO/SCIM, SLA, data warehouse, advanced governance. Scoped to your environment." },
            ].map((s) => (
        ...
        <div style={{ marginBottom: 60 }}>
          ...
          <p style={{ color: "var(--muted)", fontSize: 14.5, margin: "0 0 28px", maxWidth: 660 }}>
            Threadwire needs planners, operations, finance and customer service to participate. Per-seat pricing would undermine the cross-functional workflow that creates value.
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(230px,1fr))", gap: 14, alignItems: "start" }}>
            {TIERS.map((t) => (
              <div key={t.key} className="tf-panel" style={{ padding: 22, border: t.highlight ? "1px solid var(--amber)" : "1px solid var(--line)", position: "relative" }}>
                {t.highlight && (
                  <div style={{ position: "absolute", top: 12, right: 12 }}><Tag tone="amber">Most popular</Tag></div>
                )}
                ...
                <button className={`tf-btn${t.highlight ? " tf-btn-primary" : ""}`} style={{ width: "100%", justifyContent: "center" }} onClick={onContact}>
                  {t.cta}
                </button>
              </div>
            ))}
        ...
        {/* INDUSTRIES */}
        <div style={{ marginBottom: 60 }}>
          <div className="tf-eyebrow" style={{ marginBottom: 10 }}>Industries</div>
          <h2 className="tf-disp" style={{ fontSize: 28, fontWeight: 800, margin: "0 0 24px" }}>Built for discrete manufacturers</h2>
        ...
        <div className="tf-panel" style={{ padding: "20px 22px", marginBottom: 60, display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", background: "var(--bg2)" }}>
          ...
          <span style={{ fontSize: 13.5, color: "var(--muted)" }}>
            <b style={{ color: "var(--ink)" }}>Explore on sample data</b> — open the Delivery calendar, Blockers or Forecast tabs above. No login required.
          </span>
          <div style={{ marginLeft: "auto", display: "flex", gap: 10 }}>
            <button className="tf-btn" onClick={() => go("visibility")}>Explore sample <ArrowRight size={15} /></button>
            <button className="tf-btn tf-btn-primary" onClick={onContact}>Get in touch</button>
          </div>
        </div>
...
function ContactModal({ onClose }) {
  const [f, setF] = useState({ company: "", name: "", email: "", phone: "", preferred: "Email", website: "" });
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({ company: f.company, name: f.name, email: f.email, phone: f.phone, preferred_contact: f.preferred, website: f.website, captcha_token: captcha }),
      });
