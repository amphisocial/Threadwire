...
def _smtp_send(subject: str, from_addr: str, to_addrs: list, body_text: str, body_html: str, reply_to: str = "") -> bool:
    try:
...
def send_invite_email(to_email: str, org_name: str, inviter_name: str, accept_url: str) -> bool:
    ...
    body_html = (
        ...
        f'<b style="color:#ff9a4d">{org}</b> on Threadwire.'
        '</p>'
        f'<a href="{accept_url}" style="display:inline-block;background:#ff9a4d;color:#150b02;font-weight:700;text-decoration:none;padding:13px 26px;border-radius:10px;font-size:15px">Accept invitation</a>'
        '<p style="color:#7a8fa8;font-size:12px;margin:24px 0 0;line-height:1.5">'
...
def send_contact_email(company: str, name: str, email: str, phone: str,
    ...
    if not recipients:
        print("SMTP contact: no recipients configured", flush=True)
        return False
    ...
    body_html = (
        ...
        f'<tr><td style="padding:7px 0;color:#7a8fa8;width:160px">Company</td><td style="color:#f0f4f8;font-weight:600">{company}</td></tr>'
        f'<tr><td style="padding:7px 0;color:#7a8fa8">Name</td><td style="color:#f0f4f8">{name}</td></tr>'
        f'<tr><td style="padding:7px 0;color:#7a8fa8">Email</td><td><a href="mailto:{email}" style="color:#ff9a4d">{email}</a></td></tr>'
        f'<tr><td style="padding:7px 0;color:#7a8fa8">Phone</td><td style="color:#b8c5d6">{phone_display}</td></tr>'
        f'<tr><td style="padding:7px 0;color:#7a8fa8">Preferred contact</td><td style="color:#b8c5d6">{preferred_contact}</td></tr>'
        f'<tr><td style="padding:7px 0;color:#7a8fa8">Website</td><td style="color:#b8c5d6">{website_display}</td></tr>'
        '</table>'
        '</div></div></div>'
    )
